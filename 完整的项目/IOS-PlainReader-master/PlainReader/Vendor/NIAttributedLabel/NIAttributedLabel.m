...
@property (nonatomic, strong) FLAnimatedImage *fl_image;
...
- (CGSize)boxSize {
    ...
    return CGSizeMake(self.fl_image.size.width / scale + self.margins.left + self.margins.right,
                      self.fl_image.size.height / scale + self.margins.top + self.margins.bottom);
}
...
- (void)drawImages {
    ...
    for (CFIndex i = 0; i < numberOfLines; i++) {
        ...
        for (CFIndex k = 0; k < runCount; k++) {
            ...
            // use image view to display image instead of draw image to support gif image animation
            imageRect.origin.y = self.bounds.size.height - imageRect.origin.y - imageRect.size.height;
            
            //FLAnimatedImageView *imageView = [[FLAnimatedImageView alloc] initWithFrame:imageRect];
            //imageView.animatedImage = labelImage.fl_image;
...
CGFloat NIImageDelegateGetWidthCallback(void* refCon) {
    NIAttributedLabelImage *labelImage = (__bridge NIAttributedLabelImage *)refCon;
    CGFloat scale = [[UIScreen mainScreen] scale];
    return labelImage.fl_image.size.width / scale + labelImage.margins.left + labelImage.margins.right;
}
...
- (void)insertFLImage:(FLAnimatedImage *)image atIndex:(NSInteger)index
{
    NIAttributedLabelImage* labelImage = [[NIAttributedLabelImage alloc] init];
    labelImage.index = index;
//    labelImage.image = image;
    labelImage.fl_image = image;
    labelImage.margins = UIEdgeInsetsZero;
    labelImage.verticalTextAlignment = NIVerticalTextAlignmentBottom;
    if (nil == self.images) {
